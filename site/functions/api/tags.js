...
export async function onRequestGet({ request, env }) {
  ...
  const kinds = await env.DB.prepare("SELECT tag, kind, note, set_by, updated_at FROM tags").all();
  const map = {};
  for (const r of (kinds.results || [])) map[r.tag] = { kind: r.kind, note: r.note || "", set_by: r.set_by, updated_at: r.updated_at };
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === "kind") {
    ...
    if (kind === "") await env.DB.prepare("DELETE FROM tags WHERE tag = ?").bind(tag).run();
    else await env.DB.prepare("INSERT INTO tags (tag, kind, note, set_by, updated_at) VALUES (?,?,?,?,?) ON CONFLICT(tag) DO UPDATE SET kind = excluded.kind, note = excluded.note, set_by = excluded.set_by, updated_at = excluded.updated_at")
